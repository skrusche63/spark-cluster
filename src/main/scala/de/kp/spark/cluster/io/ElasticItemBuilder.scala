...
class ElasticItemBuilder {
  ...
  import de.kp.spark.cluster.io.ElasticBuilderFactory._
  
  def createBuilder(mapping:String):XContentBuilder = {
    /*
     * Define mapping schema for index 'index' and 'type'; note, that
     * we actually support the following common schema for association
     * and also series analysis: timestamp, site, user, group and item.
     * 
