package de.kp.spark.cluster.app
...
import de.kp.spark.cluster.{Configuration,RequestContext}
...
object TrainApp extends SparkService {
  ...
  protected val sc = createCtxLocal("ClusterContext",Configuration.spark)      
  protected val system = ActorSystem("ClusterSystem")
  ...
  protected def createParams(args:Array[String]):Map[String,String] = {
    ...
    val parser = new ArgotParser(
      programName = "Similarity Analysis Engine",
      compactUsage = true,
    ...
    config.value match {
      ...
      case Some(value) => {
        ...
        params += "top" -> cfg.getInt("top").toString

        params += "iterations" -> cfg.getInt("iterations").toString
...
class Handler(@transient ctx:RequestContext) extends Actor {
  ...
  def receive = {
    ...
    case req:ServiceRequest => {
      ...
      val master = context.actorOf(Props(new ClusterMaster(ctx))) 
      master ! Serializer.serializeRequest(req)

      val status = ClusterStatus.TRAINING_FINISHED
